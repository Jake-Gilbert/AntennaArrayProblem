...
public class AntennaArray {
    ...
    public double[] generateRandomSolutions (int numOfAntennae) {
        double range = (double) numOfAntennae / 2 - 0.25;
        double[] randomAntennaArray = new double[numOfAntennae];
        boolean valid = false;
        while (!valid) {
            randomAntennaArray = new double[numOfAntennae];
            for (int i = 0; i <= randomAntennaArray.length - 2; i++) {
                double temp = randomArrayPosition(range);
                randomAntennaArray[i] = temp;
                randomAntennaArray[randomAntennaArray.length - 1] = (double) numOfAntennae / 2;
        }

            Arrays.sort(randomAntennaArray);
            if (is_valid(randomAntennaArray)) {
                valid = true;
            }
        ...
        return randomAntennaArray;


    }

    public double randomArrayPosition (double range) {
        Random random = new Random();
        return random.nextDouble() * range + 0.25;
    }
    /**
